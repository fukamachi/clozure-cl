...
(defun result-bit-array (bit-array-1 bit-array-2 result)
  ...
  (let* ((typecode-1 (typecode bit-array-1))
    ...
    (flet ((bit-array-dimensions (bit-array typecode)
             ...
             (if (= typecode ppc32::subtag-bit-vector)
               (uvsize bit-array)
               (let* ((array-p (= typecode ppc32::subtag-arrayH))
                      (vector-p (= typecode ppc32::subtag-vectorH)))
...
(defsparclapfunction %simple-bit-boole ((op 0) (b1 %arg_x) (b2 %arg_y) (result %arg_z))
  ...
  (add %temp0 %ra1 %ra1)
  (add %temp0 %ra1 %ra1)
  (mov ppc32::misc-data-offset %imm0)
  (b @testw)
    (tst %imm3)
  @nextw
  (deccc %imm3)
...
(defun %displace-array (array dims size data offset)
  (let* ((typecode (typecode array))
    ...
    (let* ((flags (%svref array ppc32::vectorH.flags-cell)))
      ...
      (if array-p
        (progn
          (do ((i ppc32::arrayH.dim0-cell (1+ i)))
              ((null dims))
            (declare (fixnum i))
            (setf (%svref array i) (pop dims)))
          (setf (%svref array ppc32::arrayH.physsize-cell) size)
          (setf (%svref array ppc32::arrayH.displacement-cell) offset))
        (progn
          (if (or (not (logbitp $arh_fill_bit flags))
                  (> (the fixnum (%svref array ppc32::vectorH.logsize-cell)) size))
            (setf (%svref array ppc32::vectorH.logsize-cell) size))
          (setf (%svref array ppc32::vectorH.physsize-cell) size)
          (setf (%svref array ppc32::vectorH.displacement-cell) offset)))
      array)))
