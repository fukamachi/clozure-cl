...
/*
  This is a little trickier: if this is interrupted, we need to know whether or
  not the STORE-CONDITIONAL (cmpxchgq) has won or not.  If we're interrupted
  before the PC has reached the "success_test" label, repeat (luser the PC
  back to .SPstore_node_conditional.)  If we're at that label with the Z flag
  set, we won and (may) need to memoize.
*/
...
0:	__(movapd %fpzero,-dnode_size(%temp1))
	...
	__(cmpq %temp1,%temp0)
	__(jnz 0b)	
	__(movq %Rforeign_sp,(%temp0))
	__(movd %temp0,%Rforeign_sp)
...
local_label(stack_misc_alloc_heap_alloc_ivector):
	__(movd %Rforeign_sp,%imm0)
	__(subq $dnode_size,%imm0)
	__(movq %Rforeign_sp,(%imm0))
	__(movd %imm0,%Rforeign_sp)
...
_spentry(makestackblock)
	...
	__(jae 1f)
	__(movd %Rforeign_sp,%arg_z)
	__(subq %imm0,%arg_z)
	__(movq %Rforeign_sp,(%arg_z))
	__(movd %arg_z,%Rforeign_sp)
