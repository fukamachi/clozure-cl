...
(defun decode-arithmetic-error (xp fn rpc)
  (declare (ignore xp fn rpc))
  (values 'unknown nil))
...
(defcallback xcmain (:address xp :int signal :int function-index :int rpc :int code  :int)
  (declare (ignorable rpc))
  (let* ((fn (unless (eql 0 function-index)
               (indexed-gpr-lisp xp function-index))))
    (when (= (lisptag fn) x8664::tag-tra)
      ;; RPC is relative to the function, not the TRA.
      (setq fn (%return-address-function fn)))
    (with-xp-stack-frames (xp fn frame-ptr)
      (cond ((zerop signal)                 ;thread interrupt
             (cmain))
            ((= signal #$SIGFPE)
             (multiple-value-bind (operation operands)
                 (decode-arithmetic-error xp fn rpc)
               (let* ((condition-name
