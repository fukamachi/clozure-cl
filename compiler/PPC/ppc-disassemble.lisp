...
(defun ppc-gpr (r)
  (svref ppc32::*gpr-register-names* r))

(defun ppc-fpr (r)
  (svref ppc32::*fpr-register-names* r))
...
(defun ppc-unmacroexpand-function (name)
  (let* ((pname (string name))
         (opnum (gethash pname ppc32::*ppc-opcode-numbers*)))
  ...
  (let* ((disp-d (ppc-gpr rd))
	...
	(let* ((opcode 'addi)
	  ...
	  (if (and (not (logtest (1- (ash 1 ppc32::fixnumshift)) simm))
		   ...
		   (logbitp ra ppc-node-regs))
	    `(,opcode ,disp-d ,disp-a ',(ash val (- ppc32::fixnumshift)))
	    `(,opcode ,disp-d ,disp-a ,(if (eq val ppc32::nil-value) nil val)))))))

(defun ppc-unmacroexpand (insn)
  (let* ((expander (ppc-unmacroexpand-function (ppc32::opcode-name (lap-instruction-opcode insn))))
...
(defun find-ppc-opcode (i)
  (let* ((op (ldb (byte 6 26) i))
         (k (svref ppc32::*ppc-opcode-indices* op)))
    (declare (type (unsigned-byte 12) k)
             (type (unsigned-byte 6) op))
    (unless (= k -1)
      (dotimes (j (svref ppc32::*ppc-opcode-counts* op))
        (declare (type (unsigned-byte 10) j))
        (let* ((code (svref ppc32::*ppc-opcodes* (+ k j))))
          ...
          (unless (logbitp ppc32::operand-fake (ppc32::operand-flags operand))
            (let* ((extract-fn (ppc32::operand-extract-function operand)))
              (push (if extract-fn
                      ...
                      (ppc32::extract-default operand i))
                    vals))))
...
(defun ppc-disassemble-cr (val operand-spec)
  ...
  (let* ((width (ppc32::operand-width operand-spec))
         (crnum (ash val -2))
         (ccnum (logand val 3)))
    ...
    (do-dll-nodes (insn instructions)
      (unless (ppc-unmacroexpand insn)
        (let* ((opcode (lap-instruction-opcode insn))
          ...
          (do* ((operands (ppc32::opcode-operands opcode) (cdr operands))
		   ...
		   (opidx (ppc32::operand-index operand))
                   (val (cadr tail)))
              (declare (fixnum flags))
              (if (and (logbitp ppc32::operand-optional flags)
                       (eql 0 val))
                (rplacd tail (cddr tail))
                (progn
		  (if (and (or (eq opidx ppc32::$si)
			       (eq opidx ppc32::$nsi)
			       (eq opidx ppc32::$ui))
			   (eql val ppc32::nil-value))
		    (setf (cadr tail) nil)
			...
			(if (logbitp ppc32::$ppc-operand-absolute flags)
			  ...
			  (if (logbitp ppc32::$ppc-operand-fpr flags)
			    ...
			    (if (logbitp ppc32::$ppc-operand-vr flags) ; SVS
			      ...
			      (when (logbitp ppc32::$ppc-operand-gpr flags)
				(setf (cadr tail) (ppc-gpr val))
				(when (eq val ppc32::fn)
				  (let* ((disp (car tail)))
				    (when (and disp (typep disp 'fixnum))
				      (let* ((unscaled (+ (- ppc32::misc-data-offset) disp)))
...
(defun print-ppc-instruction (stream tabcount opcode parsed-operands)
  (let* ((name (if (symbolp opcode) opcode (ppc32::opcode-name opcode))))
    (if (keywordp name)
        ...
        (lap-instruction
         (let ((opcode (lap-instruction-opcode i))
           ...
           (push (list* (if label-p `(label ,address) address)
                        (if (symbolp opcode) opcode (ppc32::opcode-name opcode))
                        operands)
                 res)
           (setq label-p nil)))))
    (nreverse res)))
