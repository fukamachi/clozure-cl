  ...
  (let* ((cross-compiling (not (eq *host-backend* *target-backend*)))
    ...
    (dolist (immpair *ppc-lap-constants*)
      (let* ((imm (car immpair))
             (k (cdr immpair)))
        (declare (fixnum k))
        (setf (uvref constants-vector (ash (- k arch::misc-data-offset) -2))
              imm)))
...
(defun ppc-lap-constant-offset (x)
  (or (cdr (assoc x *ppc-lap-constants* :test #'equal))
      (let* ((n (+ arch::misc-data-offset (ash (1+ (length *ppc-lap-constants*)) 2))))
...
(defun ppc-subprimitive-address (x)
  (if (and x (or (symbolp x) (stringp x)))
    (let* ((info (find x *subprims* :test #'string-equal :key #'subprimitive-info-name)))
