  ...
  (let ((res 0))
    (with-macptrs (p)
      (do-consing-areas (area)
        (when (eql (%fixnum-ref area arch::area.code) arch::area-dynamic)
          (%setf-macptr-to-object p  area)
          (incf res (- (%get-unsigned-long p arch::area.high)
                       (%get-unsigned-long p arch::area.active))))))
    res))

(defun %reservedbytes ()
  (with-macptrs (p)
    (%setf-macptr-to-object p (%get-kernel-global 'arch::all-areas))
    (- (%get-unsigned-long p arch::area.high)
       (%get-unsigned-long p arch::area.low))))
...
(defun %usedbytes ()
  ...
  (let ((static 0)
	...
	(let* ((active (%fixnum-ref area arch::area.active))
	       (bytes (- (%get-unsigned-long p arch::area.active)
			 (%get-unsigned-long p arch::area.low)))
  ...
  (let ((free 0)
	...
	(when (member (%fixnum-ref area arch::area.code)
			...
			#.arch::area-cstack
                      #.arch::area-tstack))
	  (%setf-macptr-to-object p area)
	  (let ((active (%get-unsigned-long p arch::area.active))
		(high (%get-unsigned-long p arch::area.high))
		(low (%get-unsigned-long p arch::area.low)))
...
(defun %thread-stack-space (&optional (thread *current-lisp-thread*))
  ...
  (labels ((free-and-used (area)
	     (with-macptrs (p)
	       (%setf-macptr-to-object p area)
	       (let* ((low (%get-unsigned-long p arch::area.low))
		      (high (%get-unsigned-long p arch::area.high))
		      (active (%get-unsigned-long p arch::area.active))
		      (free (- active low))
		      (used (- high active)))
		 (loop
		   ...
		   (let ((low (%get-unsigned-long p arch::area.low))
		     ...
		     (incf used (- high low))))
		 (values free used)))))
    (let* ((tcr (%svref thread arch::lisp-thread.tcr-cell)))
      (if (or (null tcr)
	      (zerop (%fixnum-ref (%fixnum-ref tcr arch::tcr.cs-area))))
	(values 0 0 0 0 0 0)
	(multiple-value-bind (cf cu) (free-and-used (%fixnum-ref tcr arch::tcr.cs-area))
	  (multiple-value-bind (vf vu) (free-and-used (%fixnum-ref tcr arch::tcr.vs-area))
	    (multiple-value-bind (tf tu) (free-and-used (%fixnum-ref tcr arch::tcr.ts-area ))
...
(defun %suspend-other-threads ()
  (ff-call (%kernel-import arch::kernel-import-suspend-other-threads)
           :void))

(defun %resume-other-threads ()
  (ff-call (%kernel-import arch::kernel-import-resume-other-threads)
           :void))

(defun %lock-recursive-lock (lock)
  (with-macptrs ((p)
		 (owner (%get-ptr lock arch::lockptr.owner))
		 (signal (%get-ptr lock arch::lockptr.signal)))
    (%setf-macptr-to-object p (%current-tcr))
    (if (eql p owner)
      (incf (%get-unsigned-long lock arch::lockptr.count))
      (loop
	(when (eql 1 (%atomic-incf-ptr lock))
	  (setf (%get-ptr lock arch::lockptr.owner) p
		(%get-unsigned-long lock arch::lockptr.count) 1)
	  (return t))
...
(defun %unlock-recursive-lock (lock)
  (with-macptrs ((p)
    ...
    (when (eql 0 (decf (the fixnum (%get-unsigned-long lock arch::lockptr.count))))
      (setf (%get-ptr lock arch::lockptr.owner) (%null-ptr))
      (let* ((pending (1- (the fixnum (%atomic-swap-ptr lock 0)))))
	(declare (fixnum pending))
	(with-macptrs ((waiting (%inc-ptr lock arch::lockptr.waiting)))
...
(defun %suspend-tcr (tcr)
  (not (zerop (the fixnum 
                (ff-call (%kernel-import arch::kernel-import-suspend-tcr)
                         :unsigned-fullword (ash tcr arch::fixnumshift)
                         :unsigned-fullword)))))

(defun %resume-tcr (tcr)
  (not (zerop (the fixnum
		(ff-call (%kernel-import arch::kernel-import-resume-tcr)
			 :unsigned-fullword (ash tcr arch::fixnumshift)
			 :unsigned-fullword)))))
...
(defun %rplaca-conditional (cons-cell old new)
  (%store-node-conditional arch::cons.car cons-cell old new))

(defun %rplacd-conditional (cons-cell old new)
  (%store-node-conditional arch::cons.cdr cons-cell old new))
...
(defun atomic-push-uvector-cell (v i new)
  (let* ((cell (cons new nil))
         (offset (+ arch::misc-data-offset (ash 2 i))))
    (loop
      (let* ((old (uvref v i)))
        (rplacd cell old)
...
(defun %atomic-incf-gvector (v i &optional (by 1))
  ...
  (%atomic-incf-node by v (+ arch::misc-data-offset (ash i 2))))
...
(defun %atomic-incf-symbol-value (s &optional (by 1))
  ...
  (let* ((binding-address (%symbol-binding-address s)))
    ...
    (if (zerop binding-address)
      (%atomic-incf-node by s arch::symbol.vcell-cell)
