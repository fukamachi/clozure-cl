...
(defppclapfunction %ref-digit ((bignum arg_x) (i arg_y) (dest arg_z))
  ...
  (stw imm0 arch::misc-data-offset dest)
  (blr))

;;; BIGNUM[I] := DIGIT[0]
(defppclapfunction %set-digit ((bignum arg_x) (i arg_y) (digit arg_z))
  (la imm1 arch::misc-data-offset i)
  (lwz imm0 arch::misc-data-offset digit)
...
(defppclapfunction %bignum-sign-digit ((bignum arg_y) (digit arg_z))
  ...
  (la imm0 (- arch::misc-data-offset 4) imm0) ; Reference last (most significant) digit
  (lwzx imm0 bignum imm0)
  (srawi imm0 imm0 31)			;propagate sign bit
  (stw imm0 arch::misc-data-offset digit)
  (blr))
...
(defppclapfunction %bignum-oddp ((bignum arg_z))
  (lwz imm0 arch::misc-data-offset bignum)
  (clrlwi imm0 imm0 31)
...
(defppclapfunction %fixnum-to-bignum-set ((bignum arg_y) (fixnum arg_z))
  ...
  (stw imm0 arch::misc-data-offset bignum)
  (blr))
...
(defppclapfunction bignum-minusp ((bignum arg_z))
  ...
  (srwi imm0 imm0 31)
  (rlwimi imm0 imm0 4 27 27)
  (addi arg_z imm0 ppc::nil-value)	; return T if sign bit was clear before inversion
  (blr))
...
(defppclapfunction %add-with-carry ((r 12) (k 8) (c 4) (a 0) (i arg_x) (b arg_y) (j arg_z))
  (cmpwi cr1 j ppc::nil-value)
  (cmpwi cr0 i ppc::nil-value)
  (lwz temp0 a vsp)
  ...
  @got-a
  (beq cr1 @got-b)
  (la imm2 arch::misc-data-offset j)
  (lwzx imm2 b imm2)
  @got-b
  (lwz temp0 c vsp)
  ...
  (la vsp 16 vsp)
  (adde imm0 imm1 imm2)
  (la imm2 arch::misc-data-offset temp0)
...
(defppclapfunction %subtract-with-borrow ((r 12) (k 8) (borrow 4) (a 0) (i arg_x) (b arg_y) (j arg_z))
  ...
  (la imm1 arch::misc-data-offset temp1)
...
(defppclapfunction %multiply-and-add-harder-loop-2
  ...
  (let ((tem imm0)
        ...
        (yptr temp0))
    (lwz xptr x-ptr vsp)
    (la tem arch::misc-data-offset residx)
    (lwzx x xptr tem)
    (lwz yptr y-ptr vsp)
    ...
    (subi count count '1)
    (cmpwi count 0)
    (la tem arch::misc-data-offset yidx)   ; get yidx
    (lwzx y yptr tem) 
    (mullw prod-l x y)
...
(defppclapfunction %multiply-and-add ((r 4) (carry 0) (x arg_y) (i arg_x) (y arg_z))
  ...
  (lwz temp0 carry vsp)
  (lwz imm0 arch::misc-data-offset temp0)
  (addc imm1 imm1 imm0)
  (addze imm2 imm2)
  (stw imm2 arch::misc-data-offset temp0)
  (lwz arg_z r vsp)
  (la vsp 8 vsp)
...
(defppclapfunction %floor ((q 4) (r 0) (num-high arg_x) (num-low arg_y) (denom-arg arg_z))
  (let ((rem imm0)
	...
	(temp imm3)
	(denom imm4))
    (lwz denom arch::misc-data-offset denom)
    (lwz rem arch::misc-data-offset num-high)
    (lwz rem-low arch::misc-data-offset num-low)
    (mr temp denom)
    (sltu quo rem denom)
    (subi temp temp quo)
    (and temp temp denom)
    ...
    (not quo quo)
    (lwz temp0 q vsp)
    (stw quo arch::misc-data-offset temp0)
    (lwz arg_z r vsp)
    (la vsp 8 vsp)
    (stw rem arch::misc-data-offset arg_z)
    (blr)))
...
(defppclapfunction %logcount-complement ((bignum arg_y) (idx arg_z))
  (let ((arg imm0)
        (shift imm1)
        (temp imm2))
    (la arg arch::misc-data-offset idx)
    (lwzx arg bignum arg)
    (not. shift arg)
    (li arg_z 0)
    (if ne
...
(defppclapfunction %compare-bignum-digits ((a arg_x) (i arg_y) (j arg_z))
  (la imm1 arch::misc-data-offset i)
  (lwzx imm1 a imm1)
  (la imm0 arch::misc-data-offset j)
  (lwzx imm0 a imm0)
  (cmplw imm1 imm0)
  (li arg_z '0)
  (beqlr)
...
(defppclapfunction %bignum-logand ((idx 0) (x arg_x) (y arg_y) (dest arg_z))
  (vpop temp0)
  (la imm1 arch::misc-data-offset temp0)
  (lwzx imm0 x imm1)
  (lwzx imm2 y imm1)
  (and imm0 imm0 imm2)
  (stwx imm0 dest imm1)
...
(defppclapfunction %bignum-trim-leading-zeros ((bignum arg_x)
					       ...
					       (len arg_z))
  (add imm1 start len)
  (la imm1 (- arch::misc-data-offset 4) imm1)
  @loop
  (cmpwi cr0 len '1)
  (lwzx imm0 bignum imm1)
  (cmpwi cr1 imm0 0)
...
(defppclapfunction %shrink-bignum ((new-len arg_y) (bignum arg_z))
  (let ((old-len temp0)
    ...
    (la old-idx arch::misc-data-offset old-len)
    (la new-idx arch::misc-data-offset new-len)
    (beqlr)
    @loop
    (subi old-idx old-idx 4)
    (cmpw old-idx new-idx)
    (stwx ppc::rzero bignum old-idx)
    (bne @loop)
    (slwi header new-len (- arch::num-subtag-bits arch::fixnumshift))
    (ori header header arch::subtag-bignum)
    (stw header arch::misc-header-offset bignum)
    (blr)))
