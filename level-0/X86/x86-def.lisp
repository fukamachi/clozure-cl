...
#+ppc32-target
(defx86lapfunction %fixnum-set-u32 ((fixnum arg_x) (offset arg_y) #| &optional |# (new-value arg_z))
  (cmpwi cr0 nargs '2)
  (check-nargs 2 3)
  (bne cr0 @3-args)
  (mr fixnum offset)
...
(defx86lapfunction %set-current-vsp ((new-vsp arg_z))
  (check-nargs 1)
  (mr vsp new-vsp)
  (blr))
...
(defx86lapfunction %set-current-tsp ((new-tsp arg_z))
  (check-nargs 1)
  (mr tsp new-tsp)
  (blr))
...
(defx86lapfunction %%frame-savefn ((p arg_z))
  (check-nargs 1)
...
(defx86lapfunction %%frame-savevsp ((p arg_z))
  (check-nargs 1)
...
(defx86lapfunction %code-vector-pc ((code-vector arg_y) (pcptr arg_z))
...
#+eabi-target
(progn
  (defx86lapfunction %%ff-call ((fploads 8)
                                (single-offset 4)
                                (double-offset 0)
...
(progn
  ...
  (progn
    ...
    (defx86lapfunction %%ff-call ((monitor-exception-ports 12)
                                  (reload-sizes 8)
                                  (reload-offsets 4)
                                  (frame-size 0)			     
                                  (total-words arg_x)
  ...
  ;;; since the MV machinery may clobber IMM0.
    (defx86lapfunction %%ff-result ((spec arg_z))
      (stdu sp -160 sp)
      (ld arg_y ':void nfn)
      (cmpd cr0 spec arg_y)
  ...
  ;;; This is just here so that we can jump to a subprim from lisp.
    (defx86lapfunction %do-ff-call ((monitor arg_y) (entry arg_z))
      (cmpdi monitor nil)
